...
let nodeWindowManagerUnavailableReason: string | null = null
...
export function matchWechatType(name: string, appType: AppType) {
  ...
  const wechatName =
    appType === 'wechat' ? ['微信', '微信.app', 'WeChat', 'Weixin', 'Weixin.exe'] : ['企业微信', '企业微信.app']
...
function getWechatWindow(appType: AppType, windows: any[]): any {
  ...
  if ((appType as string) === 'whatsapp') {
    ...
    appTargetName =
      appType === 'wechat' ? ['微信', '微信.app', 'WeChat', 'Weixin', 'Weixin.exe'] : ['企业微信', '企业微信.app']
    windowTitle = appType === 'wechat' ? ['微信', 'Weixin', 'WeChat'] : ['企业微信']
  }
...
async function getWechatWindowInWin(appType: AppType): Promise<PlatformWindow | null> {
  if (!nodeWindowManagerUnavailableReason) {
    try {
      ...
      nodeWindowManagerUnavailableReason = err?.message || String(err)
      console.warn('[window-utils] node-window-manager unavailable, fallback to active-win:', nodeWindowManagerUnavailableReason)
    }
  }

  const fallbackWindows = await getOpenWindowsSafe()
  return getWechatWindow(appType, fallbackWindows) || null
}
