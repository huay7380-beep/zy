...
export class AIClient {
  ...
  async callChat(messages: Array<{ role: 'system' | 'user' | 'assistant'; content: string }>): Promise<string> {
    const data = await this.callAPI(messages)
    return this.extractText(data)
  }

  async *callChatStream(messages: Array<{ role: 'system' | 'user' | 'assistant'; content: string }>): AsyncIterable<string> {
    for await (const delta of this.callAPIStream(messages)) {
  ...
  private async *callAPIStream(messages: any[]): AsyncIterable<string> {
    const url = `${this.config.baseURL}/chat/completions`
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text()
        throw new Error(`API stream request failed: ${response.status} - ${errorText.slice(0, 200)}`)
      }

      if (!response.body) {
        throw new Error('API stream response body is empty')
      }
      ...
      const elapsed = ((Date.now() - callStart) / 1000).toFixed(1)
      if (error?.name === 'AbortError') {
        throw new Error(`AI API stream request timed out (${TIMEOUT_MS / 1000}s) after ${elapsed}s`)
      }
      throw error
    } finally {
      clearTimeout(timer)
